...
# Script completo para build, push para Docker Hub e deploy do delay worker
...
# Configurações
IMAGE_NAME="delay-worker"
TAG="${1:-latest}"
FULL_IMAGE_NAME="${DOCKER_HUB_USER}/${IMAGE_NAME}:${TAG}"
DEPLOYMENT_FILE="k8s/chatgpt-delay-worker-deployment.yaml"

log_info "🚀 Iniciando deploy completo do delay worker"
...
docker build -f Dockerfile.chatgpt-delay-worker -t "${FULL_IMAGE_NAME}" .
...
TEMP_DEPLOYMENT="/tmp/chatgpt-delay-worker-deployment-temp.yaml"
cp "${DEPLOYMENT_FILE}" "${TEMP_DEPLOYMENT}"

# Substituir placeholder do usuário no deployment se necessário
...
kubectl get deployments -n whatsapp-webhook -l app=chatgpt-delay-worker

echo ""
log_info "📋 Status dos pods:"
kubectl get pods -n whatsapp-webhook -l app=chatgpt-delay-worker
...
echo "   kubectl logs -n whatsapp-webhook -l app=chatgpt-delay-worker -f"
echo "   kubectl top pods -n whatsapp-webhook -l app=chatgpt-delay-worker"
echo "   kubectl describe deployment chatgpt-delay-worker -n whatsapp-webhook" 